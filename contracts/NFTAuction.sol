    ...
    ReentrancyGuardTransient //避免重入
{
    ...
    function initialize(
        ...
        // 参数验证
        _validateInitializeParams(_seller, _nftContract, _duration, _startPrice, _tokenId, _factory,_auctionId);
    ...
    function endAuction() public virtual nonReentrant {
        ...
        if (auctionInfo.highestBidder != address(0)) {
            ...
            (uint256 fee,uint256 sellerAmount) = _calculateFeeAndSellerAmount(auctionInfo.highestBid);
    ...
    function _calculateFeeAndSellerAmount(uint256 _amount) internal virtual returns (uint256, uint256){
        ...
        (bool success, bytes memory data) = auctionInfo.factory.call(abi.encodeWithSignature("getFee(uint256)", _amount));
        //解码data得到手续费
        if(!success){
    ...
    function _getHightestUSDValue() internal view virtual returns (uint256) {
        ...
        if (address(feed) == address(0)) {
            revert InvalidPayToken(auctionInfo.payToken);
        }
        //获取价格预言机价格
    ...
    function _calculateBidUSDValue(address _payToken, uint256 _amount) internal view virtual returns (uint256) {
        ...
        //判断预言机地址是否为0
        if (address(feed) == address(0)) {
            revert InvalidPayToken(_payToken);
        }
    ...
    function _initPriceFeeds() internal {
        //Sepolia测试网的ETH地址默认0x0 指向[0x694AA1769357215DE4FAC081bf1f309aDC325306](ETH/USD预言机地址)
        priceFeeds[address(0)] = AggregatorV3Interface(0x694AA1769357215DE4FAC081bf1f309aDC325306);
        //Sepolia测试网的USDC合约地址[0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238]指向[0xA2F78ab2355fe2f984D808B5CeE7FD0A93D5270E](USDC/USD预言机地址)
        priceFeeds[0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238] = AggregatorV3Interface(0xA2F78ab2355fe2f984D808B5CeE7FD0A93D5270E);
    }
