...
//NFT拍卖工厂 用于生产拍卖合约
contract NFTAuctionFactoryLocal is
    ...
    ReentrancyGuardTransient //避免重入
{
     ...
     * @param _feeRatio 拍卖收取的手续费比例 万分之单位 100 = 100/10000 1%  必须在 0.01% - 10% 之间
     * @param _auctionImplementation NFTAuctionLocal 逻辑合约地址
     */
    function initialize(
    ...
    function createAuction(
        ...
        uint256 _startPrice,
        address _payToken
    ) public nonReentrant returns (uint256 auctionId, address auctionContract) {
        if(_usdcAddress == address(0)){
            revert ParameterError("usdcAddress cannot be zero address");
        }
        ...
        bytes memory initData = abi.encodeWithSelector(
            NFTAuctionLocal.initialize.selector,
            _usdcAddress,
            msg.sender,
            _nftContract,
            _duration,
     ...
     */
    function getFee(uint256 _amount,address seller) public view returns (uint256) {
        return _getFee(_amount,seller);
    }


    function _getFee(uint256 _amount,address seller) internal view virtual returns (uint256) {
